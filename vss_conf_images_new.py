...
def find_and_replace_image_links(page_content, attachments):
    ...
    # Регулярное выражение для поиска тегов img и атрибутов style
    ...
    for link in image_links:
        ...
        if matching_attachment:
            ...
            # Удаление тега img и замена на новый тег <ac:image>, также убираем атрибуты style
            updated_content = re.sub(rf'<img src="{re.escape(link)}"[^>]*>', new_tag, updated_content)
            print(f"Replacing {link} with {new_tag}")
        else:
            print(f"No matching attachment found for {filename}")
    
    # Удаление лишних атрибутов после картинок, например, style="..."
    updated_content = re.sub(r'style="[^"]*"', '', updated_content)
